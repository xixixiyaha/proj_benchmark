...
public class ThriftCartClientImpl implements Closeable {
    public final TTransport transport;
    public final TProtocol protocol;
    public final CartService.Client client;
    private static int clientNum = 0;
    public ThriftCartClientImpl(String host, int port) {

        transport = new TFramedTransport(new TSocket(host, port));
        protocol = new TBinaryProtocol(transport);
        client = new CartService.Client(protocol);
        try {
            transport.open();
