...
public class LaunchSystem{
    ...
    private final static File resultFolder = new File("LaunchSystem-result");


    private final static List<String> funOrder = Arrays.asList("CompareResEfficiencyBM1", "BM2CompareParallelRpcEfficiency","BM3CompareChainNested","BM3CompareChainSequence","BM4ComparePatternFanout","BM4ComparePatternTrigger","BM5CompareTransferDataSize","BM6CompareMemBindWidth");
    ...
    public static void main(String[] args) throws Exception {
        ...
        // //TODO Notice Here
        allTasks.forEach(LaunchSystem::benchmark);
    ...
    private static void benchmark(String taskName) {
        try {
            ...
            var accountServerPackage = packageAndGet(new File("com.freeb.thrift-accounts"));
            var cartServerPackage = packageAndGet(new File("com.freeb.thrift-carts"));
            var categoryServerPackage = packageAndGet(new File("com.freeb.thrift-category"));
            var discountServerPackage = packageAndGet(new File("com.freeb.thrift-discount"));
            var orderServerPackage = packageAndGet(new File("com.freeb.thrift-order"));
            var paymentServerPackage = packageAndGet(new File("com.freeb.thrift-payment"));
            var productServerPackage = packageAndGet(new File("com.freeb.thrift-product"));
            ...
            startServer(accountServerPackage,"bm-account-server");
            System.out.println("<benchmark server> [bm-account-server]");

//            startLocalServer(accountServerPackage);
            //等服务器启动起来在启动客户端
            TimeUnit.SECONDS.sleep(10);

            startClient(frontendPackage);

            stopServer(accountServerPackage,"bm-account-server");
    ...
    private static void startServer(File serverPackage,String hostname) throws Exception {
        ...
        //Notice 远程执行.sh 载入/etc/hosts
        //TODO@ CHECK 这句话对不对
        exec("","scp ./importServerIP.sh"+" benchmark@"+hostname+":~",null);
        var loadIPCommand = "bash ~/importServerIP.sh";
        exec("ssh", "benchmark@"+hostname, loadIPCommand);
